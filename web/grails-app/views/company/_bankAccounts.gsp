<div class="property-value" aria-labelledby="${label}-label">
  <ul>
    ...
    <li class="subList"> 
    <g:link action="edit" controller="bankAccount" params="[company:company.id, companyBankAccount:true]" id="${bankAccount.id}">${bankAccount.accountNumber} - ${bankAccount.banco}</g:link>
    </li>
    </g:each>
  </ul>
</div>
